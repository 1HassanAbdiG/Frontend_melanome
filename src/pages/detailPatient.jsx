...
const PatientSearchInterface = () => {
  ...
  const handleSearchSubmit = async (event) => {
    ...
    try {
      const response = await axios.get(`https://wooded-classic-manchego.glitch.me/patient/${idPatient}`);
  ...
  return (
    <div className={styles.conteneur}>
      ...
      <div className={styles.resultat}>
        ...
        {patientData && patientData.length === 0 && <p className={styles.col}>Aucun patient trouvé.</p>}
      </div>
    </div>
  );
};
